Attribute VB_Name = "COASMain_V83"
' 项目：COASMain
' 版本：V83
' 作者：冯振华
' 单位：山东省平原县第一中学
...
' 日志：在编写标准VBA工具箱的过程中，找到了确定工作表最大行和最大列的方法，所以对其取行列的部分进行了优化，逐步减少对RowMax 和 ColMax 的依赖，最终将消除这两个参数，暂时不升级版本号 时间：2022/1/17
' 日志：完全去除对RowMax 和 ColMax 的常数值设置，升级为自动判断行和列，于是不再对Office版本有限制，升级版本号V83 时间：2022/1/17 21:10
Public Const COAVersion As Integer = 83
...
Public OutSource As Variant
Public Source1 As Variant
Public Source2 As Variant
...
Public HolidayAX As Variant
Public HolidayBX As Variant
Public HolidayA As Variant
Public HolidayB As Variant
...
' 在桌面创建配置文件快捷方式,暂时设定，于1月10日上班重新修改
    ...
    ' 统一调入配置表
        ...
        ViRmax = ConfigBook.Sheets(ConfigSheet1).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        ViCmax = ConfigBook.Sheets(ConfigSheet1).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
    ...
    ' 获取对时表A
        CGRmax = ConfigBook.Sheets(OriginalSheet4).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        CGCmax = ConfigBook.Sheets(OriginalSheet4).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
        ...
        SSRmax = ConfigBook.Sheets(OriginalSheet1).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        SSCmax = ConfigBook.Sheets(OriginalSheet1).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
        ...
        HRmax = ConfigBook.Sheets(ConfigSheet2).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        HCmax = ConfigBook.Sheets(ConfigSheet2).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
        ...
        RCTRmax = ConfigBook.Sheets(OriginalSheet3).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        RCTCmax = ConfigBook.Sheets(OriginalSheet3).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
        ...
        TGRmax = ConfigBook.Sheets(OriginalSheet5).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        TGCmax = ConfigBook.Sheets(OriginalSheet5).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
        ...
        Do Until TeacherGroup(1, TeGrStep + 1) > 0
            TeGrStep = TeGrStep + 1
        Loop
        STRmax = ConfigBook.Sheets(ConfigSheet3).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
        STCmax = ConfigBook.Sheets(ConfigSheet3).Range("A1").SpecialCells(xlLastCell).Column '取得最后一列
...
Sub GetSource()
    Dim DeRmax, DeCmax As Integer
' 取得原始表行数和列数
    DeRmax = Sheets(1).Range("A1").SpecialCells(xlLastCell).Row '取得最后一行
'    DeCmax = 13
    DeCmax = Sheets(1).Range("A1").SpecialCells(xlLastCell).Column
    ReDim Source(1 To DeRmax, 1 To SubColMax) As Variant
' 生成标准通用考勤数组格式
...
Sub GetQingJia() '本程序用来解析请假列表
    Dim GroupRowX As Integer
    ReDim HolidayAX(1 To SubColMax, 1 To 1) As Variant
    ReDim HolidayBX(1 To SubColMax, 1 To 1) As Variant
' 生成请假的校准表HolidayA,HolidayBX，二者均无标题，从第1行起既是有效数据，用来生成考勤统计信息
...
' 取得教师分组和对应行数
       If InStr(Holiday(i, 1), "组") > 0 Or InStr(Holiday(i, 1), "班主任") > 0 Or InStr(Holiday(i, 1), "督导室") > 0 Then
           For j = 1 To TGCmax Step TeGrStep
               ...
               End If
           Next
           GroupRowX = ConfigBook.Sheets(OriginalSheet5).Range("A1").SpecialCells(xlLastCell).Row + 1 '取得最后一行
           GroupRow = ConfigBook.Sheets(OriginalSheet5).Cells(GroupRowX, GroupColum).End(xlUp).Row
       End If
'获取日期格式请表HolidayA
                         ...
                         If Len(Holiday(i, 4)) > 0 Then
                             ...
                             Do While DateZ <= CDate(Holiday(i, 3))
                                If DateMin <= DateZ And DateZ <= DateMax Then
                                    HARowMax = HARowMax + 1
                                    ReDim Preserve HolidayAX(1 To SubColMax, 1 To HARowMax) As Variant
                                    HolidayAX(COAXingMing, HARowMax) = TeacherGroup(a, GroupColum)
                                    HolidayAX(COARiQi, HARowMax) = DateZ
                                    HolidayAX(COAQianDao, HARowMax) = Holiday(i, 4)
                                    HolidayAX(COAQianTui, HARowMax) = Holiday(i, 4)
                                End If
                                DateZ = DateZ + 1
                             Loop
                         Else
                                    ...
                                    Do While DateZ <= CDate(Holiday(i, 3))
                                        If DateMin <= DateZ And DateZ <= DateMax Then
                                            ...
                                            If InStr(Holiday(1, k), "上午") > 0 Then
                                                HolidayAX(COABanCi, HARowMax) = 1
                                            ElseIf InStr(Holiday(1, k), "下午") > 0 Then
                                                HolidayAX(COABanCi, HARowMax) = 2
                                            ElseIf InStr(Holiday(1, k), "晚上") > 0 Then
                                                HolidayAX(COABanCi, HARowMax) = 3
                                            Else
                                                MsgBox "班次出错"
                                            End If
                                            If Len(Holiday(i, k)) > 0 Then HolidayAX(5, HARowMax) = Holiday(i, k)
                                            If Len(Holiday(i, k + 1)) > 0 Then HolidayAX(6, HARowMax) = Holiday(i, k + 1)
                                         End If
                                         DateZ = DateZ + 1
                                    Loop
                                 End If
                   ...
                   If Len(Holiday(i, 4)) > 0 Then
                      ...
                      Do While DateZ <= CDate(Holiday(i, 3))
                           If DateMin <= DateZ And DateZ <= DateMax Then
                                HARowMax = HARowMax + 1
                                ReDim Preserve HolidayAX(1 To SubColMax, 1 To HARowMax) As Variant
                                HolidayAX(COAXingMing, HARowMax) = Holiday(i, 1)
                                HolidayAX(COARiQi, HARowMax) = DateZ
                                HolidayAX(COAQianDao, HARowMax) = Holiday(i, 4)
                                HolidayAX(COAQianTui, HARowMax) = Holiday(i, 4)
                           End If
                            ...
                            Do While CDate(DateZ) <= CDate(Holiday(i, 3))
                                 If DateMin <= DateZ And DateZ <= DateMax Then
                                    ...
                                    Else
                                       MsgBox "班次出错"
                                    End If
                                    If Len(Holiday(i, k)) > 0 Then HolidayAX(HARowMax, 5) = Holiday(i, k)
                                    If Len(Holiday(i, k + 1)) > 0 Then HolidayAX(HARowMax, 6) = Holiday(i, k + 1)
                                 End If
                                 DateZ = DateZ + 1
                            Loop
                          End If
...
' 获取非日期格式请假表HolidayB
               ...
               For a = 3 To GroupRow
                    If Len(Holiday(i, 4)) > 0 Then
                        ...
                        Do While WeekZ <= WeekY
                            HBRowMax = HBRowMax + 1
                            ReDim Preserve HolidayBX(1 To SubColMax, 1 To HBRowMax) As Variant
                            HolidayBX(COAXingMing, HBRowMax) = TeacherGroup(a, GroupColum)
                            HolidayBX(COAQianDao, HBRowMax) = Holiday(i, 4)
                            HolidayBX(COAQianTui, HBRowMax) = Holiday(i, 4)
                            WeekZ = WeekZ + 1
                        Loop
                    Else
                        For k = 5 To 9 Step 2
                            ...
                            If Len(Holiday(i, k)) > 0 Or Len(Holiday(i, k + 1)) > 0 Then
                                Do While WeekZ <= WeekY
                                    ...
                                    HolidayBX(COAXingMing, HBRowMax) = TeacherGroup(a, GroupColum)
                                    If InStr(Holiday(1, k), "上午") > 0 Then
                                        HolidayBX(COABanCi, HBRowMax) = 1
                                     ElseIf InStr(Holiday(1, k), "下午") > 0 Then
                                        HolidayBX(COABanCi, HBRowMax) = 2
                                     ElseIf InStr(Holiday(1, k), "晚上") > 0 Then
                                        HolidayBX(COABanCi, HBRowMax) = 3
                                     Else
                                        MsgBox "班次出错"
                                     End If
                                     HolidayBX(COAZhou, HBRowMax) = WeekZ
                                    If Len(Holiday(i, k)) > 0 Then HolidayBX(COAQianDao, HBRowMax) = Holiday(i, k)
                                    If Len(Holiday(i, k + 1)) > 0 Then HolidayBX(COAQianTui, HBRowMax) = Holiday(i, k + 1)
                                    WeekZ = WeekZ + 1
                                Loop
                            End If
                         Next
           ...
           Else
                If Len(Holiday(i, 4)) > 0 Then
                    ...
                    Do While WeekZ <= WeekY
                        HBRowMax = HBRowMax + 1
                        ReDim Preserve HolidayBX(1 To SubColMax, 1 To HBRowMax) As Variant
                        HolidayBX(COAXingMing, HBRowMax) = Holiday(i, 1)
                        HolidayBX(COAQianDao, HBRowMax) = Holiday(i, 4)
                        HolidayBX(COAQianTui, HBRowMax) = Holiday(i, 4)
                        WeekZ = WeekZ + 1
                    Loop
                       ...
                       If Len(Holiday(i, k)) > 0 Or Len(Holiday(i, k + 1)) > 0 Then
                           Do While WeekZ <= WeekY
                               ...
                               ElseIf InStr(Holiday(1, k), "晚上") > 0 Then
                                   HolidayBX(COABanCi, HBRowMax) = 3
                               Else
                                   HolidayBX(COABanCi, HBRowMax) = Holiday(1, k)
                               End If
                               HolidayBX(COAZhou, HBRowMax) = WeekZ
                               If Len(Holiday(i, k)) > 0 Then HolidayBX(COAQianDao, HBRowMax) = Holiday(i, k)
                               If Len(Holiday(i, k + 1)) > 0 Then HolidayBX(COAQianTui, HBRowMax) = Holiday(i, k + 1)
       ...
       End If
    Next
    HolidayA = Application.Transpose(HolidayAX)             '因添加数组只能添加列，所以先以列添加，最后转置一下就可以了
    HolidayB = Application.Transpose(HolidayBX)
End Sub
...
Sub COAGenerateEXE()
    ...
    ReDim Source1(1 To UBound(Source, 2), 1 To S1RowMax) As Variant
    ReDim Source2(1 To UBound(Source, 2), 1 To S2RowMax) As Variant
    For j = 2 To SRowMax
        Select Case Source(j, COAZhiWu)
            Case Is = 1
                S1RowMax = S1RowMax + 1
                For k = 1 To UBound(Source, 2)
                    ReDim Preserve Source1(1 To UBound(Source, 2), 1 To S1RowMax) As Variant
                    Source1(k, S1RowMax) = Source(j, k)
                Next
            Case Is = 2
                S2RowMax = S2RowMax + 1                                                                             '输出职务2（班主任）
                For k = 1 To UBound(Source, 2)
                    ReDim Preserve Source2(1 To UBound(Source, 2), 1 To S2RowMax) As Variant
                    Source2(k, S2RowMax) = Source(j, k)
                Next
        End Select
    Next
' 只输出异常考勤
...
Sub GenerateBook(InSource, InRmax, InCmax, InName)
    ...
    Dim InSX As Variant
    InSX = Application.Transpose(InSource)
    ReDim OutSource(1 To UBound(InSX, 1), 1 To UBound(InSX, 2)) As Variant
...
'汇总统计结果并过滤合格人员
    ...
    k = 0: p = 1
    For i = 2 To InRmax - 1
        If InSX(i, COAXingMing) = InSX(i + 1, COAXingMing) Then
            k = k + 1
        Else
            For j = i - k To i - 1
                For m = COAShangChi To InCmax
                   InSX(i, m) = InSX(i, m) + InSX(j, m)
                   InSX(j, m) = Empty
                Next
            Next
            For m = COAShangChi To InCmax
                   InSX(i, InCmax + 1) = InSX(i, InCmax + 1) + InSX(i, m)
            Next
            If InSX(i, InCmax + 1) > 0 Then
                For j = i - k To i
                    p = p + 1
                    For q = 1 To SubColMax
                        OutSource(p, q) = InSX(j, q)
                    Next
