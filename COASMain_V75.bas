Attribute VB_Name = "COASMain_V75"
' 项目：COASMain
' 版本：V75
' 作者：冯振华
' 单位：山东省平原县第一中学
...
'
Public Const COAVersion As Integer = 75
...
' 据对时表DuiShiA,DuiShiB校准Source
        ...
        For j = 1 To DSRAmax
            If InStr(Source(i, COAXingMing), DuiShiA(j, COAXingMing)) > 0 And InStr(Source(i, COARiQi), DuiShiA(j, COARiQi)) > 0 And InStr(Source(i, COABanCi), DuiShiA(j, COARiQi)) > 0 Then
                If Len(Source(i, COAQianDaoSe)) = 0 And Len(Source(i, COAQianDao)) > 0 Then
                    Source(i, COAQianDao) = CDate(Source(i, COAQianDao)) + CDate(DuiShiA(j, COAQianDaoP)) - CDate(DuiShiA(j, COAQianDaoM))
                End If
                If Len(Source(i, COAQianTuiSe)) = 0 And Len(Source(i, COAQianTui)) > 0 Then
                ...
                DuiShiTemp(1, 5) = DuiShiA(j, COAQianDaoP)
                DuiShiTemp(1, 6) = DuiShiA(j, COAQianDaoM)
                DuiShiTemp(1, 7) = DuiShiA(j, COAQianTuiP)
                DuiShiTemp(1, 8) = DuiShiA(j, COAQianTuiM)
                m = 1
            End If
        Next
        If m = 0 Then
            For j = 1 To DSRBmax
                If InStr(Source(i, COAXingMing), DuiShiB(j, COAXingMing)) > 0 And InStr(Source(i, COABanCi), DuiShiB(j, COABanCi)) > 0 And InStr(Source(i, COAZhou), DuiShiB(j, COAZhou)) > 0 Then
                    ...
                    DuiShiTemp(1, 5) = DuiShiB(j, COAQianDaoP)
                    DuiShiTemp(1, 6) = DuiShiB(j, COAQianDaoM)
                    DuiShiTemp(1, 7) = DuiShiB(j, COAQianTuiP)
                    DuiShiTemp(1, 8) = DuiShiB(j, COAQianTuiM)
                    m = 1
                End If
            Next
        End If
' 生成考勤情况
        Select Case Source(i, COAZhiWu)
            Case Is = 1                                                                         '考核教师,不考核晚上
                Select Case Source(i, COABanCi)
                    Case Is = 1                                                                 '考核上午
                         If Len(Source(i, COAQianDaoSe)) = 0 Then
                            If Len(Source(i, COAQianDao)) = 0 Then
                                Source(i, COAShangLou) = 1: Source(i, COAQianDao) = "漏签": Source(i, COAQianDaoSe) = 4
                            Else
                                If Source(i, COAQianDao) <= CDate(Standard(2, 3)) Then Source(i, COAQianDaoSe) = 1
                                If CDate(Standard(2, 3)) < CDate(Source(i, COAQianDao)) And CDate(Source(i, COAQianDao)) <= CDate(Standard(2, 4)) Then Source(i, COAQianDaoSe) = 2
                                If CDate(Standard(2, 4)) < CDate(Source(i, COAQianDao)) Then Source(i, COAShangChi) = 1: Source(i, COAQianDaoSe) = 3
                            End If
                         End If
                         If Len(Source(i, COAQianTuiSe)) = 0 Then
                            If Len(Source(i, COAQianTui)) = 0 Then
                                Source(i, COAShangLou) = Source(i, COAShangLou) + 1: Source(i, COAQianTui) = "漏签": Source(i, COAQianTuiSe) = 4
                            Else
                                If CDate(Standard(2, 6)) < CDate(Source(i, COAQianTui)) Then Source(i, COAQianTuiSe) = 1
                                If CDate(Standard(2, 5)) < CDate(Source(i, COAQianTui)) And CDate(Source(i, COAQianTui)) <= CDate(Standard(2, 6)) Then Source(i, COAQianTuiSe) = 2
                                If CDate(Source(i, COAQianTui)) <= CDate(Standard(2, 5)) Then Source(i, COAShangTui) = 1: Source(i, COAQianTuiSe) = 3
                            End If
                         End If
                    Case Is = 2                                                                                 '考核下午
                         If Len(Source(i, COAQianDaoSe)) = 0 Then
                            If Len(Source(i, COAQianDao)) = 0 Then
                                Source(i, COAXiaLou) = 1: Source(i, COAQianDao) = "漏签": Source(i, COAQianDaoSe) = 4
                            Else
                                If Source(i, COAQianDao) <= CDate(Standard(3, 3)) Then Source(i, COAQianDaoSe) = 1
                                If CDate(Standard(3, 3)) < CDate(Source(i, COAQianDao)) And CDate(Source(i, COAQianDao)) <= CDate(Standard(3, 4)) Then Source(i, COAQianDaoSe) = 2
                                If CDate(Standard(3, 4)) < CDate(Source(i, COAQianDao)) Then Source(i, COAXiaChi) = 1: Source(i, COAQianDaoSe) = 3
                            End If
                         End If
                         If Len(Source(i, COAQianTuiSe)) = 0 Then
                            If Len(Source(i, COAQianTui)) = 0 Then
                                Source(i, COAXiaLou) = Source(i, COAXiaLou) + 1: Source(i, COAQianTui) = "漏签": Source(i, COAQianTuiSe) = 4
                            Else
                                If CDate(Standard(3, 6)) < CDate(Source(i, COAQianTui)) Then Source(i, COAQianTuiSe) = 1
                                If CDate(Standard(3, 5)) < CDate(Source(i, COAQianTui)) And CDate(Source(i, COAQianTui)) <= CDate(Standard(3, 6)) Then Source(i, COAQianTuiSe) = 2
                                If CDate(Source(i, COAQianTui)) <= CDate(Standard(3, 5)) Then Source(i, COAXiaTui) = 1: Source(i, COAQianTuiSe) = 3
                            End If
                         End If
                End Select
                If m = 1 Then                                                                                      '恢复签到签退时间
                    If CInt(Source(i, COAQianDaoSe)) < 4 Then Source(i, COAQianDao) = CDate(Source(i, COAQianDao)) - CDate(DuiShiTemp(1, 5)) + CDate(DuiShiTemp(1, 6))
                    If CInt(Source(i, COAQianTuiSe)) < 4 Then Source(i, COAQianTui) = CDate(Source(i, COAQianTui)) - CDate(DuiShiTemp(1, 7)) + CDate(DuiShiTemp(1, 8))
                End If
                    ...
                    Case Is = 1                                                                                     '考核上午
                         If Len(Source(i, COAQianDaoSe)) = 0 Then
                            If Len(Source(i, COAQianDao)) = 0 Then
                                Source(i, COAShangLou) = 1: Source(i, COAQianDao) = "漏签": Source(i, COAQianDaoSe) = 4
                            Else
                                If CDate(Source(i, COAQianDao)) <= CDate(Standard(2, 9)) Then Source(i, COAQianDaoSe) = 1
                                If CDate(Standard(2, 9)) < CDate(Source(i, COAQianDao)) And CDate(Source(i, COAQianDao)) <= CDate(Standard(2, 10)) Then Source(i, COAQianDaoSe) = 2
                                If CDate(Standard(2, 10)) < CDate(Source(i, COAQianDao)) Then Source(i, COAShangChi) = 1: Source(i, COAQianDaoSe) = 3
                            End If
                         End If
                         If Len(Source(i, COAQianTuiSe)) = 0 Then
                            If Len(Source(i, COAQianTui)) = 0 Then
                                Source(i, COAShangLou) = Source(i, COAShangLou) + 1: Source(i, 6) = "漏签": Source(i, COAQianTuiSe) = 4
                            Else
                                If CDate(Standard(2, 12)) < CDate(Source(i, COAQianTui)) Then Source(i, COAQianTuiSe) = 1
                                If CDate(Standard(2, 11)) < CDate(Source(i, COAQianTui)) And CDate(Source(i, COAQianTui)) <= CDate(Standard(2, 12)) Then Source(i, COAQianTuiSe) = 2
                                If CDate(Source(i, COAQianTui)) <= CDate(Standard(2, 11)) Then Source(i, COAShangTui) = 1: Source(i, 22) = 3
                            End If
                         End If
                    Case Is = 2                                                                                     '考核下午
                         If Len(Source(i, COAQianDaoSe)) = 0 Then
                            If Len(Source(i, COAQianDao)) = 0 Then
                                Source(i, COAXiaLou) = 1: Source(i, COAQianDao) = "漏签": Source(i, COAQianDaoSe) = 4
                            Else
                                If Source(i, COAQianDao) <= CDate(Standard(3, 9)) Then Source(i, COAQianDaoSe) = 1
                                If CDate(Standard(3, 9)) < CDate(Source(i, COAQianDao)) And CDate(Source(i, COAQianDao)) <= CDate(Standard(3, 10)) Then Source(i, COAQianDaoSe) = 2
                                If CDate(Standard(3, 10)) < CDate(Source(i, COAQianDao)) Then Source(i, COAXiaChi) = 1: Source(i, 21) = 3
                            End If
                         End If
                         If Len(Source(i, COAQianTuiSe)) = 0 Then
                             If Len(Source(i, COAQianTui)) = 0 Then
'                                Source(i, COAXiaLou) = Source(i, COAXiaLou) + 1: Source(i, COAQianTui) = "漏签": Source(i, COAQianTuiSe) = 4
                             Else
                                 If CDate(Standard(3, 12)) < CDate(Source(i, COAQianTui)) Then Source(i, COAQianTuiSe) = 1
                                 If CDate(Standard(3, 11)) < CDate(Source(i, COAQianTui)) And CDate(Source(i, COAQianTui)) <= CDate(Standard(3, 12)) Then Source(i, COAQianTuiSe) = 2
'                                If CDate(Source(i, COAQianTui)) <= CDate(Standard(3, 11)) Then Source(i, COAXiaTui) = 1: Source(i, COAQianTuiSe) = 3
                             End If
                         End If
                    Case Is = 3                                                                                     '考核晚上
                         If Len(Source(i, COAQianDaoSe)) = 0 Then
                            If Len(Source(i, COAQianDao)) = 0 Then
'                                Source(i, COAWanLou) = 1: Source(i, COAQianDao) = "漏签": Source(i, COAQianDaoSe) = 4
                            Else
                                If CDate(Source(i, COAQianDao)) <= CDate(Standard(4, 9)) Then Source(i, COAQianDaoSe) = 1
                                If CDate(Standard(4, 9)) < CDate(Source(i, COAQianDao)) And CDate(Source(i, COAQianDao)) <= CDate(Standard(4, 10)) Then Source(i, COAQianDaoSe) = 2
'                                If CDate(Standard(3, 10)) < CDate(Source(i, COAQianDao)) Then Source(i, COAWanChi) = 1: Source(i, COAQianDaoSe) = 3
                            End If
                         End If
                         If Len(Source(i, COAQianTuiSe)) = 0 Then
                            If Len(Source(i, COAQianTui)) = 0 Then
                                Source(i, COAWanLou) = Source(i, COAWanLou) + 1: Source(i, COAQianTui) = "漏签": Source(i, COAQianTuiSe) = 4
                            Else
                                If CDate(Standard(4, 12)) < CDate(Source(i, COAQianTui)) Then Source(i, COAQianTuiSe) = 1
                                If CDate(Standard(4, 11)) < CDate(Source(i, COAQianTui)) And CDate(Source(i, COAQianTui)) <= CDate(Standard(4, 12)) Then Source(i, COAQianTuiSe) = 2
                                If CDate(Source(i, COAQianTui)) <= CDate(Standard(4, 11)) Then Source(i, COAWanTui) = 1: Source(i, COAQianTuiSe) = 3
                            End If
                         End If
                End Select
                If m = 1 Then                                                                                         '恢复签到签退时间
                    If 0 < Source(i, COAQianDaoSe) And Source(i, COAQianDaoSe) < 4 Then Source(i, COAQianDao) = CDate(Source(i, COAQianDao)) - CDate(DuiShiTemp(1, 5)) + CDate(DuiShiTemp(1, 6))
                    If 0 < Source(i, COAQianTuiSe) And Source(i, COAQianTuiSe) < 4 Then Source(i, COAQianTui) = CDate(Source(i, COAQianTui)) - CDate(DuiShiTemp(1, 7)) + CDate(DuiShiTemp(1, 8))
                End If
...
'    Range(Cells(1, 1), Cells(RowMax, SubColMax)) = HolidayA
'    Sheets.Add After:=ActiveSheet
'    Range(Cells(1, 1), Cells(RowMax, SubColMax)) = HolidayB
'    Sheets.Add After:=ActiveSheet
'    Range(Cells(1, 1), Cells(UBound(Source, 1), UBound(Source, 2))) = Source
''''''''''''''''''''''''''''''''''''''''''''''''
'冻结首行，方便在电脑上对照查看
